...
Dữ liệu có sẵn nhiều định dạng:
...
# Tải dữ liệu lô tô (2 số cuối)
df = pd.read_csv('https://raw.githubusercontent.com/t-k-minh/vietnam-lottery-xsmb-analysis/refs/heads/main/data/hcm-2-digits.csv')
```
...
- Cặp số đứng đầu = hay về nhất trong 1 năm

**Ý nghĩa:** Xem cặp số nào có xu hướng xuất hiện thường xuyên nhất.
...
**Ý nghĩa:** Xem phân bố tần suất có đều không. Nếu distribution tập trung quanh mean = các cặp số xuất hiện đều. Nếu có outliers = có số bất thường.

## Cấu trúc thư mục

```
data/
  hcm.json              # Dữ liệu thô TP. HCM
  hcm.csv
  hcm.parquet
  hcm-2-digits.json     # Dữ liệu 2 số cuối
  hcm-sparse.json       # Ma trận tần suất
  ...                   # Tương tự cho 20 đài còn lại

images/
  hcm/
    heatmap.jpg         # Bản đồ nhiệt
    top-10.jpg          # Top 10 số hay về
    distribution.jpg    # Phân phối tần suất
  ...                   # Tương tự cho các đài khác

src/
  run.py                # Chạy fetch + analyze (1 lệnh)
  backfill.py           # Fetch tất cả đài, 1 năm dữ liệu
  lottery.py            # Class Lottery - fetch & parse dữ liệu
  analyze.py            # Tạo biểu đồ phân tích
  fetch.py              # Fetch dữ liệu (dùng riêng lẻ)
  dtos.py               # Data model XSMNResult
  stations.py           # Lịch quay 21 đài
```